...
**** 冰风洞实验
     :LOGBOOK:
     CLOCK: [2021-02-27 Sat 11:04]--[2021-02-27 Sat 11:29] =>  0:25
     CLOCK: [2021-02-27 Sat 10:30]--[2021-02-27 Sat 10:55] =>  0:25
     :END:
...
***** 冰风洞装置原理和优势
      ...
      参考开题报告
      中航工业武汉航空仪表有限责任公司于1999年3月建成了国内首座也是目前唯一一座闭口回流式亚音速仪表冰风洞\cite{xingyuming:2011fdj}，实验段口径180mm×280mm，实验段气流速度达205米/秒，气流温度最低至$-35^{\circ}C$，液态水含量0.2~3g/m3，能够承担四大类仪表及相关系统的冰风洞实验，并获得国防科学技术奖一等奖。该风洞主要用来进行了全静压受感器冰风洞实验技术研究。
      风力机和制冷机提供冰风洞必须的风速和温度。
      冰风洞稳定段前装有大容量的冷却器，稳定段中设有可控制的喷雾装置，以便模拟真实飞行时遇到的结冰云雾条件。
      风洞风扇前设置防护网，防止冰块打伤风扇叶片。
      冰风洞轮廓图如图所示。

      经过试验段的气流首先经过第一扩散段，降低试验段中高速气流的速度以降低风洞中非试验段中的能量损失。
      气流由于摩擦、拐弯、分离等原因，在风洞内循环一周后会产生能量损失，造成一定的压力降低，而动力段的功能就是在试验段获得期望的气流速度。
      再经过后过渡段和第二扩散段后，气流在冷凝段后通过换热器降温，使风洞具有模拟结冰低温。
      再经过第三第四拐角，气流在通过4次90°拐弯后完成循环，形成了一个回路。
      冰风洞的喷雾装置安装在稳定段，根据不同试验的需要，控制喷雾的粒径和时间。
      冰风洞试验段实拍图如图3.6所示。

      衡量冰风洞的主要参数是温度、液态水含量、水滴直径三个。这些是按照 FAR25 附录 C 来确定的。温度范围 0~-30,冰云水含量 0.2~3g/m3,冰粒子直径 5~50um 不等，风速跨度较广，如低速、亚音速、跨音速、超音速风洞。

      冰风洞试验具有安全性高、成本低廉、可模拟的结冰条件范围广、结冰参数控制精确、试验重复性好等优点,是防冰性能验证的最佳途径。
      但每次冰风洞实验成本，对于传感器研制而言是昂贵的。冰风洞开始运行之后，每分钟600元，平均每天的实验成本在20万。
      因此，当前光纤式结冰传感器基本没有冰风洞实验数据，本文是第一次介绍相关数据的，这也是本课题组的贡献。
***** 四（二）种结冰条件
      验证光纤式结冰传感器安装在飞机上面对实际工况的稳定性，进行了此次冰风洞试验，
      采集的试验数据同时可以用来对冰厚计算模型进行进一步的修正。

      根据实际飞机机翼参数制作的缩比机翼模型如图所示，机翼安装在风洞实验段，气流从右侧零攻角喷向机翼。
      在机翼迎风面顶端弦向布置两个小孔，安装传感器探头。

      冰风洞进行结冰实验的流程是，降温，吹风，吹水，停止，测量冰厚。

      本次冰风洞试验条件主要有四种典型结冰条件，LWC为0.2

      | 试验工况                               | 喷水时间s                 | 试验次数 |
      |----------------------------------------+---------------------------+----------|
      | 静温-10、总温-1、粒径40μm、风速140m/s  | 25/42/60/70/95/80/120/153 |        8 |
      | 静温-10、总温-5、粒径20μm、风速100m/s  | 120/300                   |        5 |
      | 静温-20、总温-10、粒径20μm、风速140m/s | 15/18/20/25/26/38         |        7 |
      | 静温-20、总温-10、粒径20μm、风速140m/s |                           |          |

      冰风洞冰厚测量是通过三维扫描建模的方式进行的，使用激光扫描仪扫描整个机翼的形状，对比结冰前和结冰后的三维模型差别，即可得到机翼每个位置的冰厚。

      分析一波工况的选择和变化趋势。
***** 电压冰厚曲线
      这里，必须能够证明可以进行冰厚测量，冰型探测

      在冰风洞中对四种典型结冰条件进行了实验，得到的电压数据。
      同种实验条件的数据具有相似的变化趋势和电压大小，但数值并不完全一致，这主要是冰风洞的实验条件并不能达到完全一致，导致冰型发生变化。
      但数值差距很小，因此可以认为冰型缓慢变化，对应的电压变化趋势也是缓变的。

      不同实验条件的电压曲线差异明显，用数据的方差除以均值的方式来表征实验数据的离散程度，可以看出，随着温度降低，粒径降低，离散程度增加，冰型越来越接近淞冰。

      因此，可以得出结论，同一种工况下，电压曲线稳定，从喷水开始到喷水结束电压都有响应，可以灵敏反应冰厚的变化，可以在标定之后用于测量冰厚。
      不同的工况，电压变化趋势差异明显，可以有效区分不同冰型，进一步预测结冰条件。
**** 误差分析与优化
     :LOGBOOK:
     CLOCK: [2021-02-27 Sat 11:34]--[2021-02-27 Sat 12:29] =>  0:55
     :END:
***** 系统标定实验
      冰厚怎么标定？线性插值与实际不符，尤其是没有实际值

      根据冰风洞的实验数据，对传感器响应进行标定的工作，主要是根据两路电压信息，根据公式计算得到冰厚。

      因此标定实验共有三个物理量，电压1，电压2，和冰厚，
      由于没有实时冰厚测量，真实的冰厚信息只有在最后停水阶段测量得到的一组。
      基于经验，我们可以认为冰厚线性匀速增长，因此设定开始喷水时冰厚为0，喷水阶段的冰厚是起始冰厚到最终冰厚的线性插值。

      结冰过程中得到的一系列三维点，可以通过拟合工具，得到三维的拟合公式。
      最后拟合结果的可视化如图所示。

      拟合过程采用多项式拟合，电压1三阶电压2两阶。

      观察三维点在三维空间的分布情况，可以看出，实验数据仅包括了几个聚类，拟合曲面大部分仍没有真实的数据。
      根据数据的分布情况，可以认为不同冰型对应的电压数据在三维域空间中是缓变的，因此，该拟合公式，具有一定的泛化能力，即，当出现没有实验数据的冰型时，如果该冰型与某种已经拟合的冰型相差不大，仍是可以得到较准确的结果。

      因此，标定实验应该尽可能多地加入更多的实验数据。

      同时，我们分析了，仅有两路电压数据是否足够得到冰厚信息？
      其实，只有一路信息就够了，但由于明冰电压曲线是钟形曲线，不单调，一个电压值可能对应多个冰厚，无法标定。但在三维域中，可以看到曲线是单调的，这就极大方便了拟合过程。
      同时，第二路电压可以在冰厚较大时仍有比较明显的反应，扩大量程。
***** 结冰条件的正确率
      光纤式结冰传感器的精度，是喷水停止时，传感器预测的冰厚，和三维建模得到的冰厚的比较。

      在冰风洞实验中，拟合后的传感器精度如图所示。

      当前结冰测量的传感器精度要求是0.5mm，可以认为，传感器完全满足相关要求。
**** 本章小结
     本章介绍了传感器的设计要点，包括传感器的机械、电路、软件设计，实现了传感器能够得到稳定的光强信息的目标。
     在实验室搭建了结冰平台，用高低温箱、喷枪、空气压缩机等模拟了飞机的结冰环境，将传感器固定在高低温箱中，过冷水雾以高速碰撞传感器探测面，冰层快速增长冻结，得到的电压时间曲线和之前的实验数据一致，通过了传感器原理验证实验。
     为进一步验证传感器的冰厚测量能力，在武汉冰风洞中进行了四种典型结冰条件的结冰实验，经过标定后的电压可以准确测量冰厚，同时，根据电压变化趋势和电压最大值可以判断冰型，并进一步推测结冰环境。

     但，传感器不能在任意结冰条件下测量冰厚，因为不同冰型的电压拟合公式不同，采用的三维曲面拟合方法，仍然有大量的空白区域没有经过真实数据的标定。
     考虑到冰型的渐变和曲面拟合的泛化能力，在明冰和淞冰之间，找到几种典型的混合冰，用来表示某一类冰型的电压特性，即可完善拟合公式，而非需要在所有的环境中得到标定数据。
     怎么选择典型混合并及其分类，将在下一章中进行。
*** TODO 结冰实验数据处理
**** 引言
     :LOGBOOK:
     CLOCK: [2021-02-27 Sat 14:45]--[2021-02-27 Sat 15:10] =>  0:25
     :END:
     对于光纤式结冰传感器，不同的冰型会有不同的电压冰厚曲线，因此，传感器固有的冰型敏感特性的给传感器的标定和精度提升带来了巨大困难。
     前面的工作，都只讨论了几种常见结冰条件下传感器的性能，在飞机实际航行中遇到的情况中，只是极小的一部分。
     拟合公式在遇到没有标定的冰型，给出的冰厚值与实际可能想去甚远，因此，该传感器目前仅用在实验阶段，无法搭载在飞机上。

     光纤式飞机结冰探测的问题，从现象上可以描述为，飞机航行的环境中一种可以导致飞机结冰的环境，该环境包含很多可能的条件和较大的范围，被定义在飞机结冰包线中。当飞机遭遇了结冰环境，在特定位置，某种冰型的冰层以某种速率开始逐层增加。当用光纤传感器探测时，光电信号会随冰层厚度和冰型而变化。
     我们把光纤式飞机结冰探测的问题，
     用数学的方式归纳，可以表述为：
     有三个不同的域，分别是表示结冰条件的气象域，表示结冰过程的结冰域，和表示传感器数据的光电域。
     气象域中有四个变量：温度、水滴粒径、液态水含量、风速。这四个变量可能的值分布情况和相关程度，如飞机包线图所示。
     结冰域中有两个变量：结冰速率和孔隙率。这两个变量的可能取值，应该是二维平面的某个或者某几个圈。
     光电域中有两个变量：电压1和电压2。这两个变量的可能取值是无数条曲线。

     在冰风洞实验中，我们仅仅是在气象域中选取了4个点，从结冰包线来看，能够代表部分经典的结冰条件，得到了结冰域中的4个点，在光电域中，由于重复了多次实验，得到了30条曲线。
     因此，该光纤式结冰传感器在实际的工作中，其检测能力有限，主要原因是冰风洞实验代价昂贵，进行实验的结冰条件过少。

     为了提高传感器的适应能力，我们对电压数据进行了进一步的分析，改变传统数据拟合的方式，使用 SVR 对电压冰厚数据进行了回归，实现了极小的误差；
     对于不同实验条件下的数据，采用 SVM 的方式进行分类，在每一个小的气象域中进行分析，对应到不同程度的混合冰分类，在该区域中电压变化具有一致性，可以实现更准确的结冰条件预测。
**** 数据后处理
     :LOGBOOK:
     CLOCK: [2021-02-27 Sat 17:05]--[2021-02-27 Sat 17:35] =>  0:30
     :END:
***** SVR 电压冰厚拟合
      最小二乘法数据点拟合提供了一种高效可计算的方法来估计过程的状态，并使估计均方差最小，它利用前一时刻的估计值和现时刻的观测值来更新对状态变量的估计，求出现时刻的估计值，它更适合于实时处理和计算机运算。
      但在系统建模过程中，由于模型简化、噪声统计特性不准确、实际系统的参数发生变动等诸多因素的存在，使得系统模型往往存在一定的不确定性。因此，数据拟合对于模型不确定性的鲁棒性很差，导致状态估计不准，甚至出现发散。

      支持向量机回归模型在小样本情况下仍具有较快的收敛速度和计算精度，且解收敛于全局最优，克服了实际应用中样本不足的缺陷以及神经网络的过学习、局部极小值和泛化能力差的问题，使结果更接近于真实值。
      同时，在时间序列预测中，支持向量机与反映趋势变化的点紧密相关，因而支持向量机能较好的跟踪时间序列的发展趋势，即便在较长区间预测中仍可保证具有相当的精度。
      缺点是计算时间较长，实时性较差。

      对冰风洞实验的数据进行 SVR 训练，随机选取部分数据作为测试集，测试结果从对平均绝对误差(Mean Absolute Error，MAE)、均方误差(Mean Square
Error，MSE)、平均绝对百分比误差(Mean Absolute Percentage Error，MAPE)、均方百分比误差(Mean Square Percentage Error，MSP)四种预测性能指标，与最小二乘法进行了比较。如下图所示

***** SVM 结冰条件判断
      SVR 对已经分类的数据有很好的拟合效果，但对所有的实验数据进行 SVR 之后，拟合效果变差，如图所示。
      这是由于数据过于离散，且不同冰型的数据的无法有效区分。解决方法可以是训练更加复杂的网络，或者对冰型分类之后再训练。
      由于更加复杂的网络，在嵌入式系统上的无法达到相关的算力，我们选择对冰型数据分类。

      为了对冰型进行更合理的分类，本文不再采用之前的明冰、混合冰、和淞冰等根据透明程度的分类，根据实验条件和电压曲线，将所有在实验过程中出现的冰型，分为10类。
      分类首先是根据实验条件，将邻近气象域中的结冰数据都先分为一个大类，一共有6个分类，然后在同一类中，将电压变化趋势有明显差异的再细分。
      电压变化趋势标准主要是：是否先升后降、电压1和电压2趋势是否相同、是否先上升后稳定、两路电压峰值大小。

      对已经手工分类的数据，进行 SVM 学习。测试的结果如下图所示，能够对测试集进行正确分类。

      由于 SVR 的方式不是针对所有的冰型，因此，把所有的实验数据，进行 SVM 分类之后，再进行 SVR 拟合，这样拟合的结果，能够实现完全自动的更高精度的结冰测量。
**** 不同程度混合冰分类
     上一节，已经将混合冰分为10类，并且分类之后的数据，进行SVR能够得到更加效果。
     但是，这种分类不具有一致性，只能在该型号的光纤式传感器上应用。
     因此，我们讨论，将结冰进行更加细致分类，能否推进结冰探测的相关研究。

     从数学的角度出发，将结冰域分割为更多的小区域，就可以建立结冰子区域到气象域更加简洁直观的映射关系，同时，光电域的数据也会有更加明显的聚类。
     因此，进行不同程度的混合并分类是非常有必要的。

     综合之前的研究，我们提出，根据透光度，即，用光源直射冰层，另一端的光电探测器的光强与光源光强的比值，作为分类的可量化指标。
     从透光度0%到100%，分为10个子类，每10%为一个子类，成为n号混合冰。

     根据这样的分类，我们对电压数据进行了重新分类，并再次用SVM和SVR进行训练，结果取得了和手动分类接近的精度，如图所示。
**** 本章小结
     为了扩展传感器的泛化能力，能够对没有标定的结冰情况也能正常检测，采用了新的数据处理方式。
     SVR 对已经分类的数据，具有很好的拟合能力，但所有数据一起处理，会导致精度变差。
     对所有数据进行手工分类之后，SVR 可以实现高精度测量，之后进一步讨论对不同结冰程度分类的重要性与意义，可以建立结冰域与光电域和气象域之间的映射，提出并验证了新的混合冰细化分类方法。
     最终，实现了自动分类并高精度测量冰厚的光纤式飞机结冰传感器。
*** 总结与展望
    :LOGBOOK:
    CLOCK: [2021-02-27 Sat 19:07]--[2021-02-27 Sat 19:42] =>  0:35
    :END:
**** 总结
***** 全文工作总结
      飞机结冰严重危害飞机安全。
      飞机结冰探测是飞机安全领域一个重要研究难题，且是飞机试航审定中不可缺少的环节。
      本文提出了一种结构简单、测量稳定的光纤式结冰探测传感器，巧妙地将光纤传光理论和冰的物理性质结合起来，实验室的原理性验证实验保证了传感器的正常工作和稳定性，冰风洞实验证明了传感器在典型结冰环境中具有良好的精度和灵敏度。
      为了深入了解结冰过程的光学行为，建立光纤的光传输模型和冰层的光调制模型，基于斯涅尔折射定律和菲涅尔方程，对光的折射、衰减和散射等行为进行了量化分析，得到不同冰型和冰厚对反射光强的调制能力。
      并通过 COMSOL 几何光学仿真，得到了不同冰型的电压冰厚曲线，与理论曲线基本一致，证明了理论的正确性。
      冰风洞实验的数据用 SVR 的方式进行拟合，相比较于传统的最小二乘法，可以实现更好的拟合效果和更高的精度。
      本文提出了一种更加细化的混合冰分类方法，对所有数据进行了SVM分类之后再使用SVR拟合，可以达到最好的效果。
      并提出了新的冰型分类原则。
***** 本文工作主要创新点
      冰风洞实验
      SVR数据处理
      混合冰分类
***** 研究中发现的问题
      光纤式结冰传感器，其固有的冰型敏感，导致不同冰型的冰厚电压曲线不同，增加了冰厚测量的难度，降低了冰厚测量稳定性。
      但冰型测量在飞机飞行中也是非常有价值的信息，可以提供结冰气象条件信息，帮助飞行决策系统作出更加符合情况的决策。

      多维信息融合的方式进行冰厚测量，虽然可以提高系统测量的精度，但不仅在拟合过程增加了难度，计算过程也需要更多的算力，常见的嵌入式系统很难胜任大量的计算工作，并且运算时间大大增加，无法保证测量的及时准确性，在军用飞机等高实时性系统上反而得不偿失。

      光强调制型传感器容易受探测器表面的油污灰尘干扰，造成零点电压的漂移。
      在实际工作环境中，无法保证探测面的清洁，这对光纤式传感器是极大的考验。
**** 展望
     本论文虽然在光纤式结冰冰型检测技术方面做出了具有较强创新性的研究，然而在纤结冰传感器的理论、设计和试验等方面仍有许多后续工作需要开展， 这些工作至少包括以下几点：

     一是结合冰风洞实验数据，建立更准确的光纤结冰传感器和冰层的光学物理模型，为定量分析传感器的输入输出特性提供理论指导。
     本文分析了光纤结冰传感器检测冰型的基本工作原理，仅仅给出了不同冰型下的传感器特性曲线的特征，对主要的折射、衰减和散射等行为进行了量化分析，
     由于明冰、淞冰和混合冰的分类方法是很粗略的，因此为了定量探测冰型，需要优化模型中的简化和部分假设，建立更完善的光纤结冰传感器的数学物理模型，计算推导冰型、冰厚与传感器输出信号及变化率的映射关系，并通过结冰试验验证或修正。

     二是本文设计开发斜端面光纤结冰传感器中采用的红外 LED（SE5455）和光敏三极管（SE5443）在军用温度范围内的温度稳定性偏低，对传感器特性造成会较大 的影响。
     即使未能遭遇过冷大水滴，传感器长时间处于低温环境也可能具有变化的输出信号，这可以由红外 LED 和光敏三极管的光电或电光转换效率的变化引起。
     因此，后续工作应该设法在所要求的温度范围内稳定红外 LED 的发光功率和光敏三极管的灵敏度，或采取一定的温度补偿措施。

     三是本文在结冰试验中利用数码显微镜离线测量结冰厚度，从方法来说是简单有效的，但是也有诸多弊端。
     例如：①存在较大的人为误差、随机误差；②每次测 量前需打开高低温试验箱窗口，造成箱内温度的波动和结冰状态的变化；
     ③工作效率较低，冰厚数据偏少；4仅能观测到边缘位置冰厚，无法准确测量冰层凸起或凹陷的情况。
     因此，可以考虑利用特殊的机械装置固定数码显微镜，利用计算机控制它，连续地或定时地在线测量冰厚，避免上述弊端。

     四是补充完成更多结冰条件下冰风洞光纤结冰传感器的加工研制和试验研究工作。

*** 致谢
    在论文即将完稿时，回眸研究生学习和工作中的点点滴滴，感慨自己在专业领域进步的同时，也收获许多学习和生活中的感悟。
    在将近三年的研究生求学经历中，我充分认识到了科研活动的一般过程和方法，基本掌握了在所属领域独立开展创造性科学研究和技术开发工作的专业本领。
    既品尝过刻苦努力之后成功的喜悦，也体验过因天真和松懈而导致失败的痛苦，而吸取失败的惨痛教训其实也是一种进步。在此，有太多感谢的话要说。

    首先，要衷心地感谢我的导师葛俊锋副教授。葛老师在科研项目和学术论文中给了我许多具体有益的指导，他积极乐观的工作态度和循循善诱的指导方法让我深受教益。除了“传道授业解惑”，他还在生活和个人未来发展等方面给了我很多的关心和帮助。

    其次，要深深感谢在实验室朝夕相处的叶林教授和桂康博士后。
    叶老师的批判性思维方式和文质彬彬的待人接物风范，让我耳熏目染。
    桂康博士后严谨的治学态度、刻苦的工作精神、精益求精的科研作风和灵活多变的实际工作能力，都给了我很大的有益影响。

    再次，要感谢一起负责结冰项目的任宏宇、李洪宇、周浩和刘进一同学，一起在实验室研究结冰理论，研制传感器，一起讨论方案，互相争论，一起在冰风洞做实验的日子是那么难忘，感恩这个艰辛且快乐的项目我们一起前行。
    也要感谢在同一实验室学习的陈诗柄、陈前焱、崔慧英、张凯、毛俊伟、陈彬、王永阳和王丽等同学，实验室大家庭让我感受到了集体的温暖，在与他们谈理想、谈人生的过程中，我学习到了同龄人的优秀品质，逐渐对自己的未来有了更清醒的认识和定位。
    要感谢实验室的范博龙、肖汀、和戴卫中三位工程师，在传感器设计和制造以及实验阶段，提出建设性意见，弥补我们在工程实践中的不足。

    最后，还要特别感谢我的父母，感谢你们二十多年来为支持我的学业而付出的艰辛和作出的牺牲。

在临近毕业之际，怀着一颗感恩的心，祝福所有指导、帮助和支持我学业的人 一生平安幸福。
